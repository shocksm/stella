...
public class DBObjectTreeComposite extends Composite
	implements TreeListener, MouseTrackListener, MenuListener, DBObjectListener, SelectionListener, KeyListener, DisposeListener, FontChangeListener, MouseListener {
	...
	@Override
	public void catalogDataAvailable(final List<String> tableTypes, final List<String> catalogs) {
	...
	@Override
	public void tableDataAvailable(final String catalog, final String tableType, final List<TableInfo> tableInfoList) {
		this.getDisplay().asyncExec(new Runnable() {
			@Override
			public void run() {
	...
	@Override
	public void procedureDataAvailable(final String catalog, final List<ProcedureInfo> procedureInfoList) {
		this.getDisplay().asyncExec(new Runnable() {
			@Override
			public void run() {
	...
	private void processProcedureColumns(ProcedureInfo procInfo) {
		if (!tree.isDisposed()) {
			...
			if (procInfo.getColumns() != null) {
				Iterator<ProcedureColumnInfo> it = procInfo.getColumns().iterator();
				while (it.hasNext()) {
					ProcedureColumnInfo columnInfo = it.next();
					TreeItem item = new TreeItem(parentItem, SWT.NULL);
	...
	private void processProcedures(String catalog, List<ProcedureInfo> procedureInfoList) {
		if (!tree.isDisposed()) {
			...
			if (procedureInfoList == null || procedureInfoList.size() == 0) {
				...
				for (Iterator<ProcedureInfo> it = procedureInfoList.iterator(); it.hasNext();) {
					ProcedureInfo procedureInfo = it.next();
					TreeItem item = new TreeItem(parentItem, SWT.NULL);
	...
	private void processTables(String catalog, String tableType, List<TableInfo> tableInfoList) {
		if (!tree.isDisposed()) {
			...
			if (tableInfoList == null || tableInfoList.size() == 0) {
				...
				for (Iterator<TableInfo> it = tableInfoList.iterator(); it.hasNext();) {
	...
	private void processCatalogs(List<String> tableTypes, List<String> catalogs) {
		if (!tree.isDisposed()) {
			tree.removeAll();
			...
			if (catalogs != null && catalogs.size() > 0) {
				...
				while (it.hasNext()) {
					...
					Iterator<String> itTypes = tableTypes.iterator();
