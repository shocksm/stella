...
public class ResultTableComposite extends Composite implements MouseListener, DisposeListener, FontChangeListener, MenuListener, SelectionListener {
	...
	private CustomTable<Object[]> customTable = null;
	...
	public ResultTableComposite(Composite parent, String sessionName, QueryComposite qc) {
		...
		customTable = new CustomTable<Object[]>(this, true, true, true);
	...
	private void findPrimaryKeys(TableInfo tableInfo, List<Integer> resultColumnIndex) {
		if (tableInfo.getPrimaryKeys().size() > 0) {
			for (Iterator<PrimaryKeyInfo> it = tableInfo.getPrimaryKeys().iterator(); it.hasNext();) {
				PrimaryKeyInfo pkInfo = it.next();
				boolean found = false;
	...
	private void findUniqueIndex(TableInfo tableInfo, List<Integer> resultColumnIndex) {
		if (tableInfo.getIndexes().size() > 0) {
			HashSet<String> indexNameSet = new HashSet<String>();
			for (Iterator<IndexInfo> it = tableInfo.getIndexes().iterator(); it.hasNext();) {
				IndexInfo iInfo = it.next();
				if (!iInfo.getNonUnique() && iInfo.getName() != null)
					indexNameSet.add(iInfo.getName());
			}

			if (indexNameSet.size() > 0) {
				for (Iterator<String> itSet = indexNameSet.iterator(); itSet.hasNext();) {
					String indexName = itSet.next();
	...
	private void findAllColumns(TableInfo tableInfo, List<Integer> resultColumnIndex, boolean includeAll) {
		for (Iterator<ColumnInfo> it = tableInfo.getColumns().iterator(); it.hasNext();) {
	...
	private void showInsertRowInternal() {
		TableInfo tableInfo = resultData.getTableInfo();
	...
	private String generateWhereClause(List<Integer> resultColumnIndex) {
	...
	private void showUpdate() {
		if (resultData.getTableInfo().getFullyLoaded()) {
			UpdateDialog ud = new UpdateDialog(getShell(), resultData.getTableInfo(), SessionData.getSessionData(sessionName));
			ud.open();
		}
		else
			getTableData(resultData.getTableInfo(), UPDATE);
	}

	private void showInsert() {
		if (resultData.getTableInfo().getFullyLoaded()) {
			InsertDialog id = new InsertDialog(getShell(), resultData.getTableInfo(), SessionData.getSessionData(sessionName));
			id.open();
		}
		else
			getTableData(resultData.getTableInfo(), INSERT);
	}

	private void showDelete() {
		if (resultData.getTableInfo().getFullyLoaded()) {
			DeleteDialog dd = new DeleteDialog(getShell(), resultData.getTableInfo(), SessionData.getSessionData(sessionName));
			dd.open();
		}
		else
			getTableData(resultData.getTableInfo(), DELETE);
	}
	...
	private void showUpdateRow() {
		if (resultData.getTableInfo().getFullyLoaded())
			...
			getTableData(resultData.getTableInfo(), UPDATEROW);
	}
	...
	private void showDeleteRow() {
		if (resultData.getTableInfo().getFullyLoaded())
			...
			getTableData(resultData.getTableInfo(), DELETEROW);
	}
	...
	private void showInsertRow() {
		if (resultData.getTableInfo().getFullyLoaded())
			...
			getTableData(resultData.getTableInfo(), INSERTROW);
	}
	...
	private void getSchema(TableInfo tableInfo) throws SQLException {
		...
		if (tableInfo.getSchema() == null && conManager.getUseSchemas()) {
			...
			try {
				...
				String type = null;
				while (tableRs.next()) {
					if (schema == null || type.equalsIgnoreCase("synonym")) { // if last type was synonym ignore it and use this one instead
